...
bool isPointInsideViewFrustumTriangle(Vec4 const*const p){
  // -Aw <= Ai <= +Aw
  for(int i=0;i<3;++i){
...
void doPerspectiveDivisionTriangle(Vec4*const ndc,Vec4 const*const p){
  for(int a=0;a<3;++a)
...
void lobotomized_vertexPullerTriangle(GPUInVertex*const inVertex,GPUVertexPuller const*const vao,GPU const*const gpu,uint32_t vertexSHaderInvocation){
  ...
  for(int i=0; i < MAX_ATTRIBUTES; i ++){
    copyVertexAttributeTriangle(gpu,inVertex->attributes,vao->heads,vertexId);
    copyVertexAttributeTriangle(gpu,inVertex->attributes+i,vao->heads+i,vertexId);
  }
}

void lobotomized_pointRasterizationTriangle(GPUInFragment*const inFragment,Vec4 ndc,GPU*const gpu,GPUOutVertex const*const outVertex){
...
void lobotomized_perFragmentOperationTriangle(GPUOutFragment const*const outFragment,GPU*const gpu,Vec4 ndc){
  Vec4 coord = computeFragPositionTriangle(ndc,gpu->framebuffer.width,gpu->framebuffer.height);
...
void gpu_drawTriangles(GPU*const gpu,uint32_t nofVertices){
  ...
  for(uint32_t v=0;v<nofVertices;++v){
    ...
    if(!isPointInsideViewFrustumTriangle(&pos))continue;

    Vec4 ndc;
    doPerspectiveDivisionTriangle(&ndc,&pos);

    lobotomized_pointRasterizationTriangle(&fd.inFragment,ndc,gpu,&vd.outVertex);

    prg->fragmentShader(&fd);

    lobotomized_perFragmentOperationTriangle(&fd.outFragment,gpu,ndc);
  }
}
